...
public class FindMax {
	...
	public void printMax() {
		...
		Thread max1t=new Thread(()->{
			newmax1=findMax(0,29);
		});
		Thread max2t=new Thread(()->{


			newmax2=findMax(30,59);
		});
		Thread max3t=new Thread(()->{

			newmax3=findMax(60,89);
		});
		max1t.start();
		max2t.start();
		max3t.start();
		...
		int newmax=Math.max(newmax1,newmax2);
		newmax=Math.max(newmax, newmax3);


		System.out.println("the max value is " + newmax);
	}
